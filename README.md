...
* **实时单轮增量追踪**：
  ...
  * 精准掌握当前单次问答交互的体积增长。
* **🌟 官方云端实时配额与刷新倒计时 (Official Cloud Quota)**：
  * **动态自适应 RPC 探测**：自动识别本地 `language_server.exe` 端口与 CSRF 安全令牌，每 30 秒低功耗拉取官方配额；
  * **双限额独立监测**：实时呈现 **5小时滑动窗口限额** 与 **周度总限额** 剩余百分比及重置倒计时（如 `88.5% (3小时15分后刷新)`）；
  * **订阅等级与第三方模型**：直观展示当前账号会员标识（如 `💎 Google AI Pro`）与 Claude / GPT 模型额度健康度；
  * **全 HUD 原生直显**：垂直侧边栏独立展示 `5h 额度` 与 `周度额度`，横向胶囊栏与极简徽章直显 `QUOTA 88%/60%`，并在 Pro Hub 仪表盘中以高科技彩色进度条呈现，同步注入系统托盘悬停提示。
* **HUD 悬浮条与托盘深度集成**：
  * **AI 上下文容量**：横向胶囊栏与微型徽章显示当前活跃会话持有的上下文 Tokens（如 `AI 78.9k` / `7.5%`）；
  * **官方实时配额 (QUOTA)**：直接在 HUD 栏上掌握 5h/周度 剩余百分比；
  * **轻量极速架构**：全面移除多余的磁盘历史扫描，零 I/O 负担，极速响应。
* **历史会话漫游浏览器**：
